...
- **Sum-of-abs outperforms squared-sum for bilateral symmetry**: `|hipx_r| + |hipx_l|` (penalizes each joint independently) reduced mean hipx bias from 0.304 → 0.238 (22% improvement, run_020). Squared-sum `(hipx_r + hipx_l)²` allows one joint to compensate the other, weakening the signal. Apply sum-of-abs to all lateral joints that should be near zero.
- **Structural hipx/anklex asymmetry is partly correctable by reward**: With squared-sum, hipx bias only improved 6% (run_019). With sum-of-abs, 22% improvement (run_020). Left hipx persistently sits at ~-0.27 rad across all scenarios — likely a remaining structural floor from hardware frictionloss asymmetry.
